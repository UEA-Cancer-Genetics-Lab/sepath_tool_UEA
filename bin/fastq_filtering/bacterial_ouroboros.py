...
#launchsnake = ('bsub -J fastq_assembly_SEPATH -q long-eth -M 10000 -R "rusage[mem=10000]" "snakemake -s fastq_assembly_SEPATH.snake --config outdir=%s input_bam=%s --cluster \\"bsub -q {params.queue} {params.cluster}\\" --jobs 20 --latency-wait 1200 --timestamp"' %(outdir, bamfile))
...
while x<len(input_files):
	...
	launchsnake = ('bsub -J bacterial_SEPATH -q long-eth -M 10000 -R "rusage[mem=10000]" "snakemake -s  bacterial_SEPATH.snake  --config outdir=%s input_bam=%s --cluster \\"bsub -q {params.queue} {params.cluster}\\" --jobs 20 --latency-wait 1200 --timestamp"' %(outdir, bamfile))
